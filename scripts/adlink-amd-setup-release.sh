...
THIS_SCRIPT="adlink-amd-setup-release.sh"
...
if [ -z "$TEMPLATECONF" ]; then
  ...
  if [ -z "$CONF" ]; then
    ...
    if [ ! -d $PWD/meta-adlink-amd/conf/templates/default ]; then
      echo "Error: $PWD/meta-adlink-amd/conf/templates/default directory not found."
      return 1
    fi
    echo "Setup yocto environment for building adlink x86_64 target board."
    echo "    TEMPLATECONF=$PWD/meta-adlink-amd/conf/templates/default source $PROGNAME $BUILDDIR"
    echo ""
    TEMPLATECONF="$PWD/meta-adlink-amd/conf/templates/default" source $PROGNAME $BUILDDIR
  fi
else
